 ...
 * @since 2021-05-03
 */
public class VoidActivity extends BaseActivity<VoidPresenter> implements VoidContract.View {

    private final String TAG = "VoidDetailsActivity";
    private final int ACTIVITY_RESULT_REVERSAL_FAILED = 100;
    private ActivityVoidDetailsBinding binding;
    ...
    public static void startActivity(BaseActivity activity) {
        activity.startActivity(new Intent(activity, VoidActivity.class));
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DataBindingUtil.setContentView(this, R.layout.activity_void_details);
    ...
    @Override
    public void onErrorAndClear(String title, String msg) {
        showDialogError(title, msg, this::onClearVoidUI);
    }
    ...
    public void onTxnFailed(String msg) {
        FailedActivity.startActivity(msg_void_failed, msg, btn_ok, this);
        finish();
    }
    ...
    public void onInvoiceDataReceived(Transaction t, Issuer i, Merchant m, String maskedPan) {
        ...
        String amount = Utility.getFormattedAmount(Long.parseLong(t.getTotal_amount()));
    ...
    @Override
    public void gotoVoidReceiptActivity() {
        VoidReceiptActivity.startActivity(this);
        finish();
    }

    @Override
    public void invalidInvoiceNumber() {
        binding.btnContinue.setVisibility(View.GONE);
        binding.etInvoiceNo.setEnabled(false);
        binding.cvErrorMsg.setVisibility(View.VISIBLE);
        binding.incorrectInvoice.setText(Const.MSG_INCORRECT_INVOICE_NO);
        binding.ivClear.setVisibility(View.VISIBLE);
    }

    @Override
    public void showInAppError(String msg) {
        binding.btnContinue.setVisibility(View.GONE);
        binding.etInvoiceNo.setEnabled(false);
        binding.cvErrorMsg.setVisibility(View.VISIBLE);
        binding.incorrectInvoice.setText(msg);
        binding.ivClear.setVisibility(View.VISIBLE);
    }
