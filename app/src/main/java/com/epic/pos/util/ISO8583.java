...
import com.epic.pos.tlv.EMVTLVParam;
...
public class ISO8583 {
    ...
    public static final int TYPE_LL_BCD = TYPE_LEN_LEN + TYPE_BCD;
    protected static final int TYPE_L_ASC = TYPE_ASC + TYPE_LEN ;
    public static final int TYPE_LL_ASC = TYPE_ASC + TYPE_LEN_LEN ;
    protected static final int TYPE_L_BIN = TYPE_BIN + TYPE_LEN ;
    public static final int TYPE_LL_BIN = TYPE_BIN + TYPE_LEN_LEN ;
    ...
    public byte[] setField( int field, String value) {
        ...
        {
            AppLog.i(TAG, "no attribute_array set!");
            return null;
        }
        if( value.length() == 0 )
        {
            AppLog.i(TAG, "invalid value(len:0) of field:" + field);
            return null;
        }
        byte[] bytes = null;
        int len = 0;
        ...
        if( 0 != (attribute_array[field][ATTR_INDEX_TYPE] & TYPE_BCD) ) {
            ...
            AppLog.i( TAG, "type " + Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + " invalided" );
    ...
    public byte[] getFieldCust( int field, String value) {
        ...
        if( null == attribute_array)
        {
            AppLog.e(TAG, "no attribute_array set!");
            return null;
        }
        if( value.length() == 0 )
        {
            AppLog.e(TAG, "invalid value(len:0) of field:" + field);
            return null;
        }
        byte[] bytes = null;
        int len = 0;
...
//        String valueTrimed = new String(value);
        ...
        if( retBytes == null ) {
            AppLog.i( TAG, "type " + Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + ", len " + value.length + " of field: " + field+ " invalid :" + wantLen );
            validField[field] = false;
        } else {
            allField[field] = retBytes;
            validField[field] = true;
            AppLog.d( TAG, "save field:" + field + ", type " +  Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + ", len:" + value.length + ", value:" + Utility.byte2HexStr(retBytes) );
        }
        return retBytes;
    }
    ...
    public byte[] getPacket( byte[] header, byte[] tail, PACKET_TYPE type) {
        ...
        if( false == validField[1] ){
            AppLog.d(TAG, "calculate the bitmap:" );
            ...
            AppLog.d(TAG, Utility.byte2HexStr(bitmap));
        ...
        for( int i = 0; i<= ISO_BIT_MAX; i++ ){
            if( validField[i] ){
                ...
                AppLog.d( TAG, "set field " + i + ", len:" + len + ", value:" + Utility.byte2HexStr(allField[i]) );
            } else {
            }
        }
        ...
        if( false == validField[ISO_BIT_MAX] ){
            ...
            //mac = null;
            if( null != mac ){
                AppLog.d(TAG, "get mac:" + Utility.byte2HexStr(mac));
                len = 8;
                System.arraycopy( mac, 0 ,tmp, offset, len );
                offset += len ;
            } else {
                AppLog.e(TAG, "calculate mac fails" );
            }
        }
        ...
        if( type == PACKET_TYPE.PACKET_TYPE_NONE ){
            ...
            AppLog.d(TAG, "Len:" + offset + "buf size:" + packet.length );
        } else {
            return null;
        }

        AppLog.d(TAG, Utility.byte2HexStr(packet));
    ...
    public boolean unpack( byte[] packet, int offset ){
        ...
        AppLog.d(TAG, "unpack:" + Utility.byte2HexStr(packet));

        isoData = new HashMap<Integer, String>();
        unpackValidField = new boolean[ISO_BIT_MAX+1];

        AppLog.d(TAG, "message type:" + Utility.byte2HexStr( packet, index, 2) );
        ...
        AppLog.d(TAG, "packet:" + Utility.byte2HexStr(packet, index, 32));
        int fieldMark = 0;
        fieldOffset = index+8;
        --index;
        for( int i = 1; i<= ISO_BIT_MAX; i++ ){
            if( fieldMark == 0 ){
                ++index;
                fieldMark = 0x0080;
                AppLog.d(TAG, "bitmap:" + Integer.toHexString((int)packet[index])  );
            }
            if( 0 != ( fieldMark & packet[index] ) ){
                unpackValidField[i] = true;
                AppLog.d(TAG, "Mark Field:" + i);
                ...
                AppLog.d(TAG, "try read field:" + field + ", type:" + Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + ", Length:" + length );
                ...
                AppLog.d(TAG, "set field:" + field + ", type:" + Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + ", Length:" + length + ", value:" + isoData.get( field ));
    ...
    public byte[] appendF( int field, int tag, byte[] value ) {
        ...
        if (len <= 0x7F) {
            ...
            AppLog.e(TAG, "invalid length:" + len + " of TAG:" + Integer.toHexString(tag) + " to Field:" + field);
            return null;
        }
        lenTag = sTag.length();
        lenLen = offset;
        ...
        AppLog.d(TAG, "TLV:"  + Utility.byte2HexStr(TLV));
    ...
    public byte[] makePacket(Map<Integer, String> data, PACKET_TYPE type) {
        ...
        for( Map.Entry<Integer, String> field : data.entrySet() ) {
            ...
            if( tmp == null ){
                AppLog.e(TAG, "error of index:" + fieldIndex + ", value:" +  fieldValue );
            }
        }
